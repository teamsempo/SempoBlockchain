...
def process_task(self, encrypted_private_key, contract, function, args=None, kwargs=None):
    return blockchain_processor.process_task(encrypted_private_key, contract, function, args, kwargs)


@celery_app.task(base=SqlAlchemyTask, bind=True, max_retries=3, soft_time_limit=300)
def _process_function_transaction(self, transaction_id, encrypted_private_key, contract, function, args=None, kwargs=None):
    return blockchain_processor.process_function_transaction(transaction_id, encrypted_private_key, contract, function, args, kwargs)
