...
if load_from_s3:
    ...
    common_obj = client.get_object(
        Bucket=SECRET_BUCKET, Key='common_config.ini')
    ...
    folder_common_path = os.path.join(
        CONFIG_DIR, 'config_files/common_config.ini')
    folder_specific_path = os.path.join(
        CONFIG_DIR, 'config_files/' + CONFIG_FILENAME)
    ...
    common_path = folder_common_path if os.path.isfile(
        folder_common_path) else raw_common_path
    specific_path = folder_specific_path if os.path.isfile(
        folder_specific_path) else raw_specific_path
...
DEFAULT_FEEDBACK_QUESTIONS = list(
    specific_parser['APP']['DEFAULT_FEEDBACK_QUESTIONS'].split(','))
FEEDBACK_TRIGGERED_WHEN_BALANCE_BELOW = int(
    specific_parser['APP']['FEEDBACK_TRIGGERED_WHEN_BALANCE_BELOW'])
FEEDBACK_TRIGGERED_WHEN_TRANSFER_COUNT_ABOVE = int(
    specific_parser['APP']['FEEDBACK_TRIGGERED_WHEN_TRANSFER_COUNT_ABOVE'])
LIMIT_EXCHANGE_RATE = float(
    specific_parser['APP'].get('LIMIT_EXCHANGE_RATE', 1))
CASHOUT_INCENTIVE_PERCENT = float(
    specific_parser['APP'].get('CASHOUT_INCENTIVE_PERCENT', 0))
AUTO_APPROVE_TRANSFER_ACCOUNTS = specific_parser['APP'].getboolean(
    'AUTO_APPROVE_TRANSFER_ACCOUNTS', False)
MAXIMUM_CUSTOM_INITIAL_DISBURSEMENT = int(
    specific_parser['APP'].get('MAXIMUM_CUSTOM_INITIAL_DISBURSEMENT', 0))
...
REQUIRE_TRANSFER_CARD_EXISTS = specific_parser['APP'].getboolean(
    'REQUIRE_TRANSFER_CARD_EXISTS', False)

SECRET_KEY = specific_parser['APP']['SECRET_KEY'] + DEPLOYMENT_NAME
ECDSA_SECRET = hashlib.sha256(
    specific_parser['APP']['ECDSA_SECRET'].encode()).digest()[0:24]
APP_HOST = specific_parser['APP']['APP_HOST']

TOKEN_EXPIRATION = 60 * 60 * 24 * 1  # Day

INTERNAL_AUTH_USERNAME = common_parser['APP']['BASIC_AUTH_USERNAME'] + \
    '_' + DEPLOYMENT_NAME
...
DATABASE_PASSWORD = os.environ.get(
    "DATABASE_PASSWORD") or specific_parser['DATABASE']['password']

ETH_DATABASE_NAME = specific_parser['DATABASE'].get('eth_database') \
    or common_parser['DATABASE']['eth_database']

ETH_DATABASE_HOST = specific_parser['DATABASE'].get(
    'eth_host') or DATABASE_HOST

ETH_DATABASE_HOST = specific_parser['DATABASE'].get(
    'eth_host') or DATABASE_HOST
ETH_WORKER_DB_POOL_SIZE = specific_parser['DATABASE'].getint(
    'eth_worker_pool_size', 40)
ETH_WORKER_DB_POOL_OVERFLOW = specific_parser['DATABASE'].getint(
    'eth_worker_pool_overflow', 160)
...
SQLALCHEMY_DATABASE_URI = get_database_uri(
    DATABASE_NAME, DATABASE_HOST, censored=False)
CENSORED_URI = get_database_uri(DATABASE_NAME, DATABASE_HOST, censored=True)

ETH_DATABASE_URI = get_database_uri(
    ETH_DATABASE_NAME, ETH_DATABASE_HOST, censored=False)
CENSORED_ETH_URI = get_database_uri(
    ETH_DATABASE_NAME, ETH_DATABASE_HOST, censored=True)
...
PUSHER_ENV_CHANNEL = common_parser['PUSHER']['environment_channel'] + \
    '-' + DEPLOYMENT_NAME
PUSHER_SUPERADMIN_ENV_CHANNEL = common_parser['PUSHER'][
    'superadmin_environment_channel'] + '-' + DEPLOYMENT_NAME
...
ETH_TARGET_TRANSACTION_TIME = int(
    specific_parser['ETHEREUM']['target_transaction_time'] or 120)
ETH_GAS_PRICE_PROVIDER = specific_parser['ETHEREUM']['gas_price_provider']
ETH_CONTRACT_NAME = 'SempoCredit{}_v{}'.format(
    DEPLOYMENT_NAME, str(ETH_CONTRACT_VERSION))

ETH_CHECK_TRANSACTION_BASE_TIME = 20
ETH_CHECK_TRANSACTION_RETRIES = int(
    specific_parser['ETHEREUM']['check_transaction_retries'])
ETH_CHECK_TRANSACTION_RETRIES_TIME_LIMIT = sum(
    [ETH_CHECK_TRANSACTION_BASE_TIME * 2 **
        i for i in range(1, ETH_CHECK_TRANSACTION_RETRIES + 1)]
)

INTERNAL_TO_TOKEN_RATIO = float(
    specific_parser['ETHEREUM'].get('internal_to_token_ratio', 1))
FORCE_ETH_DISBURSEMENT_AMOUNT = float(
    specific_parser['ETHEREUM'].get('force_eth_disbursement_amount', 0))

unchecksummed_withdraw_to_address = specific_parser['ETHEREUM'].get(
    'withdraw_to_address')
if unchecksummed_withdraw_to_address:
    WITHDRAW_TO_ADDRESS = Web3.toChecksumAddress(
...
MASTER_WALLET_PRIVATE_KEY = specific_parser['ETHEREUM'].get(
    'master_wallet_private_key')
if MASTER_WALLET_PRIVATE_KEY:
    master_wallet_private_key = bytes.fromhex(
        MASTER_WALLET_PRIVATE_KEY.replace('0x', ''))
else:
    master_wallet_private_key = keccak(text=SECRET_KEY + DEPLOYMENT_NAME)
    MASTER_WALLET_PRIVATE_KEY = master_wallet_private_key.hex()

MASTER_WALLET_ADDRESS = keys.PrivateKey(
    master_wallet_private_key).public_key.to_checksum_address()

RESERVE_TOKEN_ADDRESS = specific_parser['ETHEREUM'].get(
...
SYSTEM_WALLET_TARGET_BALANCE = int(
    specific_parser['ETHEREUM'].get('system_wallet_target_balance', 0))
SYSTEM_WALLET_TOPUP_THRESHOLD = int(
    specific_parser['ETHEREUM'].get('system_wallet_topup_threshold', 0))

ETH_CONTRACT_TYPE = specific_parser['ETHEREUM'].get(
    'contract_type', 'standard').lower()
...
if specific_parser['ETHEREUM'].get('dai_contract_address'):
    ...
    ETH_CONTRACT_ADDRESS = specific_parser['ETHEREUM'].get(
        'dai_contract_address')

IS_USING_BITCOIN = False

EXCHANGE_CONTRACT_ADDRESS = specific_parser['ETHEREUM'].get(
    'exchange_contract_address')

SYNCRONOUS_TASK_TIMEOUT = specific_parser['ETHEREUM'].getint(
    'synchronous_task_timeout', 4)
