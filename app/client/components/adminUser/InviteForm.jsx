...
export class InviteFormContainer extends React.Component {
  ...
  onEmailFieldKeyPress(e) {
    ...
    this.setState({email: email, email_missing: false, invalid_request: false});
...
const InviteForm = function(props) {
  ...
  return(
    <div>
          ...
          {tiers.map((tier, i) => {
            return (
              <LabelContainer key={i}>
                {tier}
                <LabelInput type="radio" value={tier} checked={props.tier === tier} onChange={props.handleToggle}/>
      ...
      <AsyncButton onClick={props.onClick} isLoading={props.isLoggingIn} buttonStyle={{width: 'calc(100% - 1em)', display: 'flex'}} buttonText="Invite"/>

    </div>
  );

};export default connect(mapStateToProps, mapDispatchToProps)(InviteFormContainer);
