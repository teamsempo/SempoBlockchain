...
class VolumeChart extends React.Component {
  ...
  render() {
    ...
    const transaction_dates = this.props.creditTransferStats.daily_transaction_volume.map(
      data => new Date(data.date),
    );

    const disbursement_dates = this.props.creditTransferStats.daily_disbursement_volume.map(
      data => new Date(data.date),
    );

    const all_dates = transaction_dates.concat(disbursement_dates);

    const minDate = new Date(Math.min.apply(null, all_dates));
    const maxDate = new Date(Math.max.apply(null, all_dates));

    const date_array = getDateArray(minDate, maxDate);

    const transaction_volume = get_zero_filled_values(
      'volume',
      this.props.creditTransferStats.daily_transaction_volume,
      date_array,
    );

    const disbursement_volume = get_zero_filled_values(
      'volume',
