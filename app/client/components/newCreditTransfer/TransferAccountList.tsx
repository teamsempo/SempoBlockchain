...
const columns: ColumnsType<TransferAccount> = [
  ...
  },
  {
    ...
    ellipsis: true,
    render: (text: any, record: any) => (
      <Tag color={record.blockchain_status_colour}>{record.blockchain_status}</Tag>
...
class TransferAccountList extends React.Component<Props, ComponentState> {
  ...
  render() {
    ...
    let data: any[] = Object.values(creditTransfers.byId).map((transfer: any) => {
      const recipient_user =
        transfer.recipient_user &&
        users.byId[transfer.recipient_user];

      const sender_user =
        transfer.sender_user &&
        users.byId[transfer.sender_user];
      ...
      const recipient_name = recipient_user ?
        (recipient_user.first_name === null ? "" : recipient_user.first_name) + " " +
