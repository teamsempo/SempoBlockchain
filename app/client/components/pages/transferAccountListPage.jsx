...
class TransferAccountListPage extends React.Component {
  ...
  buildFilterForAPI() {
    if (location.pathname.includes("vendors")) {
      ...
      location.pathname.includes(window.BENEFICIARY_TERM_PLURAL.toLowerCase())
    ) {
  ...
  render() {
    ...
    const is_beneficiary = location.pathname.includes(
      window.BENEFICIARY_TERM_PLURAL.toLowerCase()
    );

    if (is_vendor || is_beneficiary) {
      // just vendors or beneficiaries
      var transferAccountList = this.props.mergedTransferAccountUserList.filter(
        transferAccount => {
          return transferAccount.is_vendor === is_vendor;
        }
      );
    } else {
