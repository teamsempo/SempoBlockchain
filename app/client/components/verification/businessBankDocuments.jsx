...
class BusinessBankDocuments extends React.Component {
  ...
  _validateData(data) {
    return  {
      bank_documents_val: data.uploaded_documents.filter(document => document.reference === 'bank').length > 0,
    }
  }

  _validationErrors(val) {
    const errMsgs = {
      bank_documents_val_msg: val.bank_documents_val ? '' : 'You must upload at least one bank document',
    };
    return errMsgs;
  }
  ...
  render() {
    ...
    return(
      <div>
        ...
        <SecondaryText>Please upload your most recent bank statement within the last three months. The document must also include your full legal name (or your company's) and match the address registered on Sempo. If your bank statement has a different address, please also provide a utility bill for your registered address.</SecondaryText>
        <br/>
        <SecondaryText>
          Please note:
        </SecondaryText>
        ...
        {documentLoading}
        {bankDocuments}
        <UploadButton handleFileChange={(e) => this.handleFileChange(e, 'bank')}/>
...
export default connect(mapStateToProps, mapDispatchToProps)(BusinessBankDocuments);
