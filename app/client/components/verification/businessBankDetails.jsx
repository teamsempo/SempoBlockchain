...
import {editBankAccount, createBankAccount, UPDATE_ACTIVE_STEP} from "../../reducers/businessVerificationReducer";
...
class BusinessBankDetails extends React.Component {
  ...
  componentDidMount() {
    ...
    if (bank_account !== null && typeof bank_account !== 'undefined') {
      Object.keys(this.state).map(key => {
        if (bank_account[key] !== null && typeof bank_account[key] !== 'undefined') {
  ...
  _validationErrors(val) {
    const errMsgs = {
      routing_number_val_msg: val.routing_number ? '' : 'Please provide a valid routing number',
      account_number_val_msg: val.account_number ? '' : 'Please provide a valid account_number',
    ...
    return(
      <div>
        <Row>
          <InputObject>
            <InputLabel>
              {businessProfile.bank_accounts[0].bank_country === 'Australia' ? 'BSB' : 'Routing Number'}
            </InputLabel>
            <ManagerInput
              name="routing_number"
              placeholder={businessProfile.bank_accounts[0].bank_country === 'Australia' ? '123123' : '123123123'}
              type="text"
        ...
        <Row>
          <InputObject>
            ...
            <StyledSelectKey name="currency" value={this.state.currency} onChange={this.handleInputChange}>
              <option name="select" value="select" disabled>Select</option>
              {currencyOptions.map((currency, index) => {
                return(
                  <option key={index} name={currency} value={currency}>{currency}</option>
                )
              })}
            </StyledSelectKey>
        ...
        <ThemeProvider theme={DefaultTheme}>
          <div style={{display: 'flex'}}>
            ...
            <AsyncButton buttonText={'Next'} onClick={this.isValidated} isLoading={this.props.editBankAccountStatus.isRequesting || this.props.createBankAccountStatus.isRequesting} buttonStyle={{display: 'flex'}}/>
          </div>
        </ThemeProvider>
...
export default connect(mapStateToProps, mapDispatchToProps)(BusinessBankDetails);
