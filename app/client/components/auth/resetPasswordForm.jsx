...
class ResetPasswordFormContainer extends React.Component {
  ...
  onNewPasswordFieldKeyPress(e) {
    ...
    this.setState({new_password: password, password_missing: false, passwordIsValid: isValid});
  }
    ...
    if (this.props.resetPasswordState.success) {
      return (
        <div style={{textAlign: 'center'}}>
          ...
          <Text>You can now <StyledLink to='/' exact>Login</StyledLink></Text>
        </div>
      )
    }
...
const ResetPasswordForm = function(props) {
  ...
  return(
    <div style={{textAlign: 'center'}}>
      ...
      <AsyncButton onClick={props.onClick} isLoading={props.isRegistering} buttonStyle={{width: 'calc(100% - 1em)', display: 'flex'}} buttonText="Change Password"/>
