...
class NewTransferManager extends React.Component {
  ...
  createNewTransfer() {
    ...
    let is_bulk = false;
    const recipient_transfer_accounts_ids = this.props.transfer_account_ids;
    const transfer_type = this.state.create_transfer_type;
  ...
  render() {
    if (this.props.login.usdToSatoshiRate) {
      const amount =
        Math.round(
          (this.state.transfer_amount / this.props.login.usdToSatoshiRate) *
            100,
        ) / 100;
