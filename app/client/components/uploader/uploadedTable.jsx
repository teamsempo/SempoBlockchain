...
import { saveDataset, resetUploadState } from '../../reducers/spreadsheetReducer'
...
class uploadedTable extends React.Component {
  ...
  componentWillMount() {
    this.dataList = Object.keys(this.props.data.table_data).map((id) => this.props.data.table_data[id]);
  }
  componentDidMount(){
  ...
  handleTableClick(e,column,rowInfo, instance) {
    ...
    if (normalised_index < 0) {
      this.setState({selectedColumn: (this.state.selectedColumn === column.id) ? null : column.id})
    } else if (normalised_index === 0) {
      this.setState({selectedColumn: (this.state.selectedColumn === column.id) ? null : column.id},() =>
      {
        let first_row_item = this.props.data.table_data[0][this.state.selectedColumn]
        if (first_row_item) {
          this.setState({customAttribute: first_row_item});
        }
      })
    } else if (normalised_index === 1) {
      this.setState({selectedRow: (this.state.selectedRow === rowInfo.index)? null : rowInfo.index })
    }
  }
  ...
  guessColumn(requested_attribute_name) {
      ...
      let column_index_guess = this.props.data.column_firstrows[requested_attribute_name];
  ...
  handleStepIncrement(increment) {
    ...
    var current_normalised_step_index = this.state.step - this.props.data.requested_attributes.length;
    ...
    if (current_normalised_step_index < 0) {
      var requested_attribute = this.props.data.requested_attributes[current_step_index];
    ...
    if (new_normalised_step_index < 0) {
      let new_requested_attribute = this.props.data.requested_attributes[new_step_index];
    ...
    this.setState({
      step: Math.min(this.state.step + increment, this.props.data.requested_attributes.length + 2)
    });
  }
  ...
  render() {
    ...
    var columnList  = Object.keys(this.dataList[0]).map((id) =>{
      return({
        Header: (id in this.state.headerPositions)? this.state.headerPositions[id] : '',
    ...
    if (this.currentStepNormalisedIndex()  === 0) {
      var stepSpecificFields = <CustomColumnFields
        ...
        handleCustomAttributeClick={(item) => this.handleCustomAttributeClick(item)}
    ...
    if (this.props.saveState.saved && this.props.saveState.diagnostics.length > 0) {

      var added = 0;
      var updated = 0;
      var errors = 0;

      var diagnostic_list = this.props.saveState.diagnostics.map((diagnostic, index) =>
        {
          if (diagnostic[1] !== 200) {
            errors += 1;
            return (
      ...
      var main_body =
        <div>
          ...
          <p>
            <b> Added: {added}, Updated: {updated}, Errors: {errors} </b>
          </p>
          { diagnostic_list }
        </div>
    } else {
      main_body = (
        <ReactTable
          ...
          getTdProps={(state, rowInfo, column, instance) => {
            ...
            if (rowInfo) {
              var background = (column.id == this.state.selectedColumn || rowInfo.index == this.state.selectedRow)? '#dff5f3' : 'white'
            } else {
              background = (column.id == this.state.selectedColumn)? '#dff5f3' : 'white'
            }

            var color = (column.id in this.state.headerPositions || this.state.firstDataRow == null) ? '#666' : '#ccc';
    ...
    return (
      <PageWrapper>
        ...
        <div style={{display: this.props.saveState.saved ? 'none' : 'flex', justifyContent: 'space-between'}}>

          <StyledButton
            onClick={() => this.handlePrevClick()}
            style={(this.state.step === 0 || this.props.saveState.isRequesting || this.props.saveState.saved) ? {opacity: 0, pointerEvents: 'None'}: {}}
          >
            Prev
          </StyledButton>

          <StyledButton onClick={() => this.handleNextClick()}
                        style={(this.props.saveState.isRequesting || this.props.saveState.saved) ? {opacity: 0, pointerEvents: 'None'}: {}}
          >
...
const CustomColumnFields = function (props) {
  if (props.selectedColumn) {
      return(
        <StepSpecificFieldsContainer>
          <CustomList customAttributes={props.customAttributes} handleClick={(item) => props.handleCustomAttributeClick(item)}/>
          <div>
            Please add a label:
          </div>
          <CustomInput
          ...
          />
          <StyledButton onClick={() => props.handleAddClick()}> Add </StyledButton>
        </StepSpecificFieldsContainer>
      )
  } else {
       return (
...
const Prompt = function(props) {
  ...
  if (props.step < props.requested_attributes.length) {
    ...
    var text = (
      `Which column contains the ${requested_key_display_name} of ${account_type}?`
    )
