...
function* loadCreditTransferFilters({payload}) {
    try {
        ...
        yield put({type: LOAD_CREDIT_TRANSFER_FILTERS_FAILURE, error: fetch_error})
    }
}

function* watchLoadCreditTransferFilters() {
    yield takeEvery(LOAD_CREDIT_TRANSFER_FILTERS_REQUEST, loadCreditTransferFilters);
}

export default function* metricSagas() {
    yield all([
        watchLoadCreditTransferFilters(),
    ])
}