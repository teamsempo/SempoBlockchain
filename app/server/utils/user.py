...
def proccess_create_or_modify_user_request(
    ...
    uppercase_account_types = [a.upper() for a in account_types]

    is_vendor = attribute_dict.get('is_vendor', None)
    if is_vendor is None:
        is_vendor = 'VENDOR' in uppercase_account_types or attribute_dict.get('vendor', False)

    is_tokenagent = 'TOKEN_AGENT' in uppercase_account_types or attribute_dict.get('is_tokenagent', False)
    is_groupaccount = 'GROUP_ACCOUNT' in uppercase_account_types or attribute_dict.get('is_groupaccount', False)

    is_beneficiary = 'BENEFICIARY' in uppercase_account_types or attribute_dict.get('is_beneficiary', False)
