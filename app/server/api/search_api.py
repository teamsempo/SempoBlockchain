...
from server.utils.search import generate_search_query, CREDIT_TRANSFER, TRANSFER_ACCOUNT
from server.schemas import transfer_accounts_schema, view_transfer_accounts_schema
from server.schemas import credit_transfers_schema, view_credit_transfers_schema
...
class SearchAPI(MethodView):
    """
        ...
        Parameters:
            - search_type: whether you're searching for TRANSFER_ACCOUNT or CREDIT_TRANSFER
    ...
    def get(self):
        ...
        final_query = generate_search_query(search_string, filters, order, sort_by_arg, search_type=search_type)
        if search_type == CREDIT_TRANSFER:
            credit_transfers, total_items, total_pages, _ = paginate_query(final_query, ignore_last_fetched=True)
